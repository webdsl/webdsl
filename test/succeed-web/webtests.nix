...
let
  ...
  build = appname :
   with import "/lib/testing.nix" {system = "i686-linux";} ;
      runInMachineWithX {
        require = [ ./machine.nix ];
        drv = pkgs.stdenv.mkDerivation {
          ...
          buildCommand = \'\'
            ensureDir 
            cp -R /share/webdsl/webdsl-check/test/succeed-web/ succeed-web/
	
            cd succeed-web
            TOPDIR=/home/chrismelman/workspaces/webdsl-build/webdsl/test/succeed-web
            FAILED=""
            export DISPLAY=:0.0
            header "Running "
            result=""
            ...
            echo "Executing 'webdsl test-web "
            webdsl test-web  2>&1 || export FAILED=1
            stopNest
            if test -z ""; then
              exit 0
            else
