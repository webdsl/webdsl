...
test DateTimeInput{
  ...
  d.runJavaScript("$('.flatpickr-calendar').hide();");
...
test EntityInput{
  ...
  var sel := Select( d.findElements(SelectBy.className("input-trigger"))[0] );
...
test EntityListInput{
  ...
  var sel := Select(d.findElements(SelectBy.tagName("select"))[0]);
...
test allInput{
  ...
  var sel := Select(d.findElements(SelectBy.tagName("select"))[1]);
  sel.selectByVisibleText("3");
  
  var fileInput := d.findElements(SelectBy.tagName("input"))[19];
  fileInput.sendKeys(createTempFile("test file"));
  
  sel := Select(d.findElements(SelectBy.className("input16"))[0]);
