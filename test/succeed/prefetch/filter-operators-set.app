...
// This test is the same as filter-operators-list.app, but using a Set instead of a List.
// As a result an order by expression is provided for the elements of the set to fix the order of the output.
...
define filterNotOr() {
  for(ent : Ent order by ent.seq) {
    for(elemNotOr : Elem in ent.col where !(elemNotOr.seq < 5 || elemNotOr.seq > 5) order by elemNotOr.seq) {
...
define filterAnd() {
  for(ent : Ent order by ent.seq) {
    for(elemAnd : Elem in ent.col where elemAnd.seq < 5 && elemAnd.seq > 5 order by elemAnd.seq) {
...
define filterEq() {
  for(ent : Ent order by ent.seq) {
    for(elemEq : Elem in ent.col where elemEq.seq == 5 order by elemEq.seq) {
...
define filterNotEq() {
  for(ent : Ent order by ent.seq) {
    for(elemNotEq : Elem in ent.col where elemNotEq.seq != 5 order by elemNotEq.seq) {
...
define filterIsNull() {
  for(ent : Ent order by ent.seq) {
    for(elemIsNull : Elem in ent.col where elemIsNull.prev == null order by elemIsNull.seq) {
...
define noFilterIsNull() {
  for(ent : Ent order by ent.seq) {
    for(elemIsNull : Elem in ent.col where elemIsNull.next == null order by elemIsNull.seq) {
      output(ent) output(elemIsNull)
    } separated-by { "|" }
  } separated-by { "#" }
}
...
define filterIsNotNull() {
  for(ent : Ent order by ent.seq) {
    for(elemIsNotNull : Elem in ent.col where elemIsNotNull.prev != null order by elemIsNotNull.seq) {
...
define noFilterIsNotNull() {
  for(ent : Ent order by ent.seq) {
    for(elemIsNotNull : Elem in ent.col where elemIsNotNull.next != null order by elemIsNotNull.seq) {
      output(ent) output(elemIsNotNull)
    } separated-by { "|" }
  } separated-by { "#" }
}
...
define filterSmallerThan() {
  for(ent : Ent order by ent.seq) {
    for(elemSmallerThan : Elem in ent.col where elemSmallerThan.seq < 2 order by elemSmallerThan.seq) {
...
define filterSmallerThanOrEqual() {
  for(ent : Ent order by ent.seq) {
    for(elemSmallerThanOrEqual : Elem in ent.col where elemSmallerThanOrEqual.seq <= 2 order by elemSmallerThanOrEqual.seq) {
...
define filterLargerThan() {
  for(ent : Ent order by ent.seq) {
    for(elemLargerThan : Elem in ent.col where elemLargerThan.seq > 7 order by elemLargerThan.seq) {
...
define filterLargerThanOrEqual() {
  for(ent : Ent order by ent.seq) {
    for(elemLargerThanOrEqual : Elem in ent.col where elemLargerThanOrEqual.seq >= 7 order by elemLargerThanOrEqual.seq) {
